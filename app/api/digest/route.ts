...
// Monthly "you're fine" digest. Returns a structured summary the UI
// (or future email sender) renders. Tone: calm, reassuring, plain.
...
export type DigestData = {
  ...
  one_liner: string;          // 'You earned X, spent Y, saved Z for tax.'
  things_to_do: string[];     // short actions
...
function periodLabel(from: Date, to: Date): string {
  const m = to.toLocaleString("en-GB", { month: "long", year: "numeric" });
  return m;
}
...
export async function GET(req: Request) {
  try {
    ...
    const monthsBack = Math.max(0, parseInt(url.searchParams.get("monthsBack") ?? "0"));

    // Period = the calendar month ending {monthsBack} months ago
    const now = new Date();
    ...
    if (await faConnected()) {
      // Pull primary business account txns for the period
      ...
      if (primary) {
        for (let page = 1; page <= 6; page++) {
          ...
          for (const t of txns) {
            const d = t.dated_on;
            if (d < periodStartISO) continue;
            if (d > periodEndISO) continue;
            inPeriod++;
            const amt = parseFloat(t.amount);
            if (amt > 0) money_in += amt;
            else money_out += Math.abs(amt);
          }
          txn_count += inPeriod;
    ...
    // Audit log activity for the same window
    ...
    if (recentSummary.auto_applied >= 10) {
      things_going_well.push("Your AI bookkeeper is keeping pace — no backlog building up.");
    }
